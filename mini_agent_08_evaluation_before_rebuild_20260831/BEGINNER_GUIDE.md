...
→ 8-6 Provider 비교 (선택)
→ 8-7 실제 LLM 응답 평가
→ 8-8 PostgreSQL에 평가 이력 저장
→ 8-9 Redis에 최근 실패 Trace 캐시
```
...
영구적인 평가 실행과 회귀 이력은 PostgreSQL에 저장하고, Redis는 최근 실패 Trace를 잠시 확인하는 TTL 캐시로 사용합니다. API Key와 전체 시스템 Prompt는 어느 저장소에도 기록하지 않습니다.
