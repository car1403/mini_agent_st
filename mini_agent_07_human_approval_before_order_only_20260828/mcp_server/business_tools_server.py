...
@mcp.tool()
def save_itinerary(city: str, place: str) -> dict:
    """승인된 여행 일정을 저장합니다. 외부 상태를 변경하는 Tool입니다."""
    item = {"itinerary_id": f"IT-{len(SAVED_ITINERARIES) + 1:04d}", "city": city.strip(), "place": place.strip()}
    SAVED_ITINERARIES.append(item)
...
def create_return_request(order_id: str, reason: str) -> dict:
    """승인된 반품 요청을 접수합니다. 외부 상태를 변경하는 Tool입니다."""
    order_id = order_id.strip().upper()
    if order_id not in ORDERS:
        return {"success": False, "order_id": order_id, "error": "ORDER_NOT_FOUND"}
    item = {
        "return_id": f"RET-{len(RETURN_REQUESTS) + 1:04d}",
        "order_id": order_id,
        "reason": reason.strip(),
        "status": "requested",
    }
    RETURN_REQUESTS.append(item)
    return {"success": True, **item}
...
def place_order(product_id: str, quantity: int) -> dict:
    """승인된 주문을 생성하고 재고를 차감합니다. 외부 상태를 변경하는 Tool입니다."""
    product_id = product_id.strip().upper()
    if quantity < 1:
        raise ValueError("quantity는 1 이상이어야 합니다.")
    data = PRODUCTS.get(product_id)
